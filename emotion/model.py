...
class EmotionNet(nn.Module):
    ...
    def forward(self,  # type: ignore
        ...
        emotion_encoder_output, emotion_attn_logits = self.emotion_encoder(
            inputs=emotion_input_embedding, sequence_length=encoder_input_length, cause_labels=cause_labels)
        ...
        if  emotion_label is not None:
            ...
            return {'emotion_loss':emotion_loss, 'cause_loss': cause_loss,'emotion_accu':emotion_accu}, emotion_preds

        else:
            emotion_preds = torch.argmax(emotion_logits, dim=-1)
            cause_preds = torch.argmax(cause_logits, dim=-1)
...
class Transformer(nn.Module):
    ...
    def _embedding_fn(self, tokens: torch.LongTensor,
                      ...
                      emotion: torch.LongTensor) -> torch.Tensor:
        ...
        emotion_embed = self.emotion_embedder(emotion)
        emotion_embed = emotion_embed.expand_as(word_embed)
        return word_embed + pos_embed + emotion_embed
    ...
    def forward(self,  # type: ignore
        ...
        src_input_embedding = self._embedding_fn(encoder_input, positions, emotion_label)
        ...
        if decoder_input is not None and labels is not None:
            ...
            top_k=10, softmax_temperature=0.7)

            cls_output = mmoe_outputs[0][:,0,:]
            emotion_logits = self.emotion_pred_layer(cls_output)
            emotion_preds = torch.argmax(emotion_logits, dim=-1)

            predictions = self.decoder(
                memory=mmoe_outputs[1],
                memory_sequence_length=encoder_input_length,
