...
class TransformerEncoder(tx.modules.TransformerEncoder):
    ...
    def forward(self,  # type: ignore
                ...
                cause_labels: torch.LongTensor,) -> torch.Tensor:
        r"""Encodes the inputs.
        ...
        cause_padding = 1- cause_labels *   sequence_mask(
            sequence_length, inputs.size()[1]).float()
        if self._hparams.use_bert_config:
        ...
        cause_self_attention_bias = attn.attention_bias_ignore_padding(cause_padding)
...
class TransformerDecoder(tx.modules.TransformerDecoder):
    def embed_tokens(self, tokens: torch.LongTensor,
        ...
        return self._token_pos_embedder(tokens, positions, emotion)
    def initialize_blocks(self):
    ...
    def forward(self,  # type: ignore
                ...
                emotion_label: Optional[torch.Tensor] = None,
        ...
        cause_padding = 1- cause_labels *   sequence_mask(
            memory_sequence_length, memory.size(1)).float()
        cause_self_attention_bias = attn.attention_bias_ignore_padding(cause_padding)
        ...
        if (helper is None and beam_width is None and
            ...
            inputs = self.embed_tokens(inputs, times, emotion_label)
            ...
            decoder_output = self._self_attention_stack(
                ...
                memory_attention_bias, cause_self_attention_bias, cache=None)
        ...
        for i in range(self._hparams.num_blocks):
            ...
            if memory is not None:
                ...
                cause_output = self.cause_attns[i](
                    ...
                    memory=memory,
                    memory_attention_bias=cause_attention_bias)
